...
# 1. API-key invoer in UI en sessie
if not SERPAPI_KEY:
...
def scrape_contactgegevens(url: str) -> dict:
    ...
    try:
        ...
        # Fallback: JavaScript-gedreven pagina
        ...
        result["functietitels"] = list({
            line.strip() for line in tekst.split("\n")
    ...
    if not {'locatienaam','plaats'}.issubset(df.columns):
        ...
        if st.button("Start scraping"):
            ...
            for idx, row in df.iterrows():
                ...
                # Foutoverzicht
                if data.get('error'): errors_df.loc[len(errors_df)] = [naam, data['error']]
                # Update UI
